...
function getProjectHomes () {
  return atom.config.get('project-manager-ide.projectHome')
...
export function filterProjects (rows, options = {}) {
  ...
  rows = _.filter(rows, row => {
    ...
    if (!atom.config.get('project-manager-ide.autoDiscover')) {
...
function projectChangeNotification (item) {
  ...
  let detail = null
  if (atom.config.get('project-manager-ide.showPath')) {
